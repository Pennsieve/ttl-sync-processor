...
func TestSync_ComputeHash(t *testing.T) {
	contrib1 := Contributor{
		FirstName:     uuid.NewString(),
		MiddleInitial: "W",
		LastName:      uuid.NewString(),
	...
	for scenario, params := range map[string]struct {
		...
		"both slice(nil)":        {[]Contributor(nil), []Contributor(nil), true},
		"different kinds of nil": {[]Contributor(nil), nil, true},
		"both empty":             {[]Contributor{}, []Contributor{}, true},
		"one empty, one nil":     {[]Contributor{}, nil, false},
	} {
		t.Run(scenario, func(t *testing.T) {
			hash1, err := ComputeHash(params.contribs1)
			require.NoError(t, err)
			hash2, err := ComputeHash(params.contribs2)
			require.NoError(t, err)
			if params.expectEqual {
				assert.Equal(t, hash1, hash2)
				fmt.Println(hash1)
